<?php
session_start(); // PORNIM sesiunea pentru coș

// Modelele de bază (la fel ca tine)
$base = [
    ['type'=>'Covor Persan','name'=>'Covor Persan Royal','price_new'=>'350 Lei','price_old'=>'400 Lei','desc'=>'Un covor persan autentic...','dimensions'=>'300 x 200 cm','material'=>'Lână naturală 100%','origin'=>'Isfahan, Iran','stock'=>'În Stoc'],
    ['type'=>'Covor Persan','name'=>'Covor Persan Imperial','price_new'=>'350 Lei','price_old'=>'-','desc'=>'Cel mai elaborat covor persan, cu motive complexe persane.','dimensions'=>'400 x 300 cm','material'=>'Lână naturală 100%','origin'=>'Iran','stock'=>'În Stoc'],
    ['type'=>'Covor Modern','name'=>'Covor Modern Abstract','price_new'=>'350 Lei','price_old'=>'-','desc'=>'Design contemporan cu motive abstracte vibrante.','dimensions'=>'250 x 180 cm','material'=>'Lână și bumbac','origin'=>'România','stock'=>'În Stoc'],
    ['type'=>'Covor Modern','name'=>'Covor Artistic Premium','price_new'=>'350 Lei','price_old'=>'450 Lei','desc'=>'O piesă de colecție cu design artistic unic.','dimensions'=>'280 x 200 cm','material'=>'Lână naturală și mătase','origin'=>'România','stock'=>'În Stoc'],
    ['type'=>'Covor Tradițional','name'=>'Covor Oriental Clasic','price_new'=>'350 Lei','price_old'=>'-','desc'=>'Eleganță clasică orientală cu motive geometrice rafinate.','dimensions'=>'350 x 250 cm','material'=>'Lână naturală','origin'=>'Turcia','stock'=>'În Stoc'],
    ['type'=>'Covor Tradițional','name'=>'Covor Traditional Moldovenesc','price_new'=>'350 Lei','price_old'=>'-','desc'=>'Inspirat din tradițiile moldovenești, cu motive florale și geometrice.','dimensions'=>'200 x 150 cm','material'=>'Lână naturală','origin'=>'România','stock'=>'În Stoc']
];
...
for($i = 0; $i < 60; $i++){
    $b = $base[$i % 6];
    $ext = ($index === 5) ? 'png' : (in_array($index, $jpgExceptions) ? 'jpg' : 'png');
    $b['name'] .= " " . $index;
    $b['img'] = "Image".$index.".".$ext;
    $items[$b['name']] = $b;
    $index++;
}

// Verificăm dacă există elementul cerut
if(!isset($_GET['item']) || !isset($items[$_GET['item']])){
    die("<h2 style='text-align:center;margin-top:40px;'>Produsul nu a fost găsit.</h2>");
}

$item = $items[$_GET['item']];

// LOGICA ADĂUGĂRII ÎN COȘ CU REDIRECȚIONARE
if(isset($_POST['add_to_cart'])){
    $qty = max(1,intval($_POST['qty']));
    $cart_item = ['name'=>$item['name'],'price'=>$item['price_new'],'qty'=>$qty,'img'=>$item['img']];
    if(!isset($_SESSION['cart'])) $_SESSION['cart'] = [];
    if(isset($_SESSION['cart'][$item['name']])){
        $_SESSION['cart'][$item['name']]['qty'] += $qty;
    } else {
        $_SESSION['cart'][$item['name']] = $cart_item;
    }
...
/* Footer General */
footer {
  background: #b5651d; /* portocaliu când nu e dark mode */
    ...
    <div class="product-info">
        ...
        <div class="benefits-simple">
            <ul>
                ...
                <li>Retur în 30 zile</li>
            </ul>
        </div>

        <!-- CASUTE COMANDA / OFERTA -->
        <div class="action-cards">
            <a href="tel:+40123456789" class="action-card">Comandă Telefonic</a>
            <a href="#" class="action-card">Solicită Ofertă</a>
        </div>

    <form method="post" style="margin-top:20px; display:flex; gap:10px; align-items:center;">
        <input type="number" name="qty" value="1" min="1" style="width:60px; padding:8px; border-radius:6px; border:1px solid #ccc;">
        <button type="submit" name="add_to_cart" class="action-card" style="flex:1; text-align:center;">
            Adaugă în coș
        </button>
    </form>
