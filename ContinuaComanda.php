...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($cart as $item) {
            ...
            if ($product_id <= 0) {
                throw new Exception("Produs invalid in cos");
            }
            ...
            if (!$prod) {
                throw new Exception("Produs inexistent ID $product_id");
            }

            if ($prod['stock'] < $qty) {
                throw new Exception("Stoc insuficient pentru produsul ID $product_id");
            }
        ...
        if ($shipping_method === 'easybox') {
            ...
            $stmt = $pdo->prepare(
                "SELECT county, city, address FROM easyboxes WHERE id = ? AND active = 1"
            );
            $stmt->execute([$easybox_id]);
            $box = $stmt->fetch();

            if (!$box) {
                throw new Exception("EasyBox inexistent");
            }
            ...
            if ($address) {
                ...
                $stmt = $pdo->prepare("
                    ...
                    (user_id, type, name, phone, county, city, address_line)
                    VALUES (?, 'shipping', ?, ?, ?, ?, ?)
                ");
                $stmt->execute([
                    $user_id,
        ...
        $stmt = $pdo->prepare("
            ...
            (user_id, total, shipping_method, easybox_id, payment_method,
             shipping_name, shipping_phone, shipping_address, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        $stmt->execute([
            $user_id,
        ...
        foreach ($cart as $item) {
            ...
            if (!$prod || $prod['stock'] < $qty) {
                throw new Exception("Eroare stoc produs $product_id");
            }
...
<div class="container">
    ...
    <form id="checkoutForm" method="post" action="ContinuaComanda.php">
        ...
        <div class="section">
            ...
            <p class="total">Total produse: <?= number_format($total, 2) ?> Lei</p>
        </div>
...
<!-- Sumare comanda -->
<div class="summary">
    <p>Cost produse: <?= number_format($total, 2) ?> Lei</p>
    <p>Cost livrare: <?= number_format(($addresses['shipping']['type'] ?? '') === 'pickup' ? 12.99 : 25, 2) ?> Lei</p>
    <strong>Total de plata: <?= number_format($total + (($addresses['shipping']['type'] ?? '') === 'pickup' ? 12.99 : 25), 2) ?> Lei</strong>
    <br><br>
    <button type="submit" class="submit-order">Trimite comanda</button>
</div>
    <!-- Alege alta adresa -->
...
if(addressForm){
    addressForm.addEventListener('submit', function(e){
        ...
        .then(data => {
            ...
            document.querySelector('.address-list').appendChild(div);
            ...
            // redirect catre ContinuaComanda.php
            window.location.href = "ContinuaComanda.php";
        })
        .catch(err => { 
            console.error(err); 
            alert('Eroare server'); 
